...
export const photosRouter = t.router({
  ...
  findUniquePhotoOrThrow: shieldedProcedure
    .input(PhotoFindUniqueSchema).query(async ({ ctx, input }) => {
      const findUniquePhotoOrThrow = await ctx.prisma.photo.findUniqueOrThrow(input);
      return findUniquePhotoOrThrow;
    }),
  groupByPhoto: shieldedProcedure
    .input(PhotoGroupBySchema).query(async ({ ctx, input }) => {
      const groupByPhoto = await ctx.prisma.photo.groupBy({ where: input.where, orderBy: input.orderBy, by: input.by, having: input.having, take: input.take, skip: input.skip });
      return groupByPhoto;
    }),
  updateManyPhoto: shieldedProcedure
    .input(PhotoUpdateManySchema).mutation(async ({ ctx, input }) => {
      const updateManyPhoto = await ctx.prisma.photo.updateMany(input);
      return updateManyPhoto;
    }),
  updateOnePhoto: shieldedProcedure
    .input(PhotoUpdateOneSchema).mutation(async ({ ctx, input }) => {
      const updateOnePhoto = await ctx.prisma.photo.update(input);
      return updateOnePhoto;
    }),
  upsertOnePhoto: shieldedProcedure
    .input(PhotoUpsertSchema).mutation(async ({ ctx, input }) => {
