...
export const storiesRouter = t.router({
  ...
  findUniqueStoryOrThrow: shieldedProcedure
    .input(StoryFindUniqueSchema)
    .query(async ({ ctx, input }) => {
      const findUniqueStoryOrThrow = await ctx.prisma.story.findUniqueOrThrow(
        input
      )
      return findUniqueStoryOrThrow
    }),
  groupByStory: shieldedProcedure
    .input(StoryGroupBySchema)
    .query(async ({ ctx, input }) => {
      const groupByStory = await ctx.prisma.story.groupBy({
        where: input.where,
