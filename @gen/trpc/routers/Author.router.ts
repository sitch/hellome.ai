...
export const authorsRouter = t.router({
  ...
  findUniqueAuthorOrThrow: shieldedProcedure
    .input(AuthorFindUniqueSchema)
    .query(async ({ ctx, input }) => {
      const findUniqueAuthorOrThrow = await ctx.prisma.author.findUniqueOrThrow(
        input
      )
      return findUniqueAuthorOrThrow
    }),
  groupByAuthor: shieldedProcedure
    .input(AuthorGroupBySchema)
    .query(async ({ ctx, input }) => {
      const groupByAuthor = await ctx.prisma.author.groupBy({
        where: input.where,
