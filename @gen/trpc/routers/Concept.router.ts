...
export const conceptsRouter = t.router({
  ...
  findUniqueConceptOrThrow: shieldedProcedure
    .input(ConceptFindUniqueSchema)
    .query(async ({ ctx, input }) => {
      const findUniqueConceptOrThrow =
        await ctx.prisma.concept.findUniqueOrThrow(input)
      return findUniqueConceptOrThrow
    }),
  groupByConcept: shieldedProcedure
    .input(ConceptGroupBySchema)
    .query(async ({ ctx, input }) => {
      const groupByConcept = await ctx.prisma.concept.groupBy({
        where: input.where,
