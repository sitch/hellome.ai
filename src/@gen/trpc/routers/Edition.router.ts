...
export const editionsRouter = t.router({
  aggregateEdition: shieldedProcedure
    .input(EditionAggregateSchema).query(async ({ ctx, input }) => {
  ...
  createManyEdition: shieldedProcedure
    .input(EditionCreateManySchema).mutation(async ({ ctx, input }) => {
  ...
  findUniqueEditionOrThrow: shieldedProcedure
    .input(EditionFindUniqueSchema).query(async ({ ctx, input }) => {
      const findUniqueEditionOrThrow = await ctx.prisma.edition.findUniqueOrThrow(input);
      return findUniqueEditionOrThrow;
    }),
  groupByEdition: shieldedProcedure
    .input(EditionGroupBySchema).query(async ({ ctx, input }) => {
      const groupByEdition = await ctx.prisma.edition.groupBy({ where: input.where, orderBy: input.orderBy, by: input.by, having: input.having, take: input.take, skip: input.skip });
      return groupByEdition;
    }),
  updateManyEdition: shieldedProcedure
    .input(EditionUpdateManySchema).mutation(async ({ ctx, input }) => {
