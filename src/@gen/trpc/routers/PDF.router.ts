...
export const pdfsRouter = t.router({
  ...
  findManyPDF: shieldedProcedure
    .input(PDFFindManySchema).query(async ({ ctx, input }) => {
      const findManyPDF = await ctx.prisma.pDF.findMany(input);
      return findManyPDF;
    }),
  findUniquePDF: shieldedProcedure
    .input(PDFFindUniqueSchema).query(async ({ ctx, input }) => {
  ...
  groupByPDF: shieldedProcedure
    .input(PDFGroupBySchema).query(async ({ ctx, input }) => {
      const groupByPDF = await ctx.prisma.pDF.groupBy({ where: input.where, orderBy: input.orderBy, by: input.by, having: input.having, take: input.take, skip: input.skip });
      return groupByPDF;
    }),
  updateManyPDF: shieldedProcedure
    .input(PDFUpdateManySchema).mutation(async ({ ctx, input }) => {
      const updateManyPDF = await ctx.prisma.pDF.updateMany(input);
      return updateManyPDF;
    }),
  updateOnePDF: shieldedProcedure
    .input(PDFUpdateOneSchema).mutation(async ({ ctx, input }) => {
      const updateOnePDF = await ctx.prisma.pDF.update(input);
      return updateOnePDF;
    }),
  upsertOnePDF: shieldedProcedure
    .input(PDFUpsertSchema).mutation(async ({ ctx, input }) => {
